...
#
# Author: Johnny Brown
# From https://github.com/theJohnnyBrown/permanova/blob/master/permanova.py
# Accessed 04/26/2023
...
def permanova_oneway(dm, levels, permutations=200):
    ...
    One-way permanova tests the null hypothesis that distances between levels of # noqa
    the variable are not significantly different from distances within levels of # noqa
    the variable.
...
# FIXME the right way to reuse code for one-way and n-way
# is to add an arg f such that
...
def f_oneway(dm, levels):
    ...
    sst = np.sum(
        stats.ss(r)
        for r in (s[n+1:] for n, s in enumerate(dm[:-1]))
    ) / float(bign)
    ...
    ssw = np.sum(
        (
            dm[i][j]**2
            for i, j in product(range(len(dm)), range(1, len(dm)))
            if i < j and levels[i] == levels[j]
        )
    ) / float(n)
...
def permanova_twoway(dm, levels, permutations=200):
    ...
    a_levels = list([level[0] for level in levels])
    b_levels = list([level[1] for level in levels])
    ...
    for i in range(permutations):
        ...
        f_i, f_a, f_b = f_twoway(
            dm,
            zip(a_levels, [level[1] for level in levels])
        )
    ...
    for i in range(permutations):
        ...
        f_i, f_a, f_b = f_twoway(
            dm,
            zip([level[0] for level in levels], b_levels)
        )
    ...
    p_i, p_a, p_b = [
        above/float(permutations)
        for above in [above_i, above_a, above_b]
    ]
...
def f_twoway(dm, levels):
    ...
    bign = len(levels)  # number of observations
    dm = np.asarray(dm)  # distance matrix
    a = len(set([level[0] for level in levels]))  # number of a-levels
    b = len(set([level[1] for level in levels]))  # number of b-levels
...
def select_ss(dm, levels,  included):
    ...
    distances = (
        dm[i][j] for i, j in above_diagonal(bign)
